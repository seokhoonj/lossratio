...
(tri.df
    ...
    .select(
        "duration",
        (pl.col("loss") / 1e6).round(1).alias("loss_mn"),
        (pl.col("premium") / 1e6).round(1).alias("prem_mn"),
        pl.col("ratio").round(3),
    ))
#> shape: (1, 4)
#> ┌──────────┬─────────┬─────────┬───────┐
#> │ duration ┆ loss_mn ┆ prem_mn ┆ ratio │
#> │ ---      ┆ ---     ┆ ---     ┆ ---   │
#> │ i64      ┆ f64     ┆ f64     ┆ f64   │
#> ╞══════════╪═════════╪═════════╪═══════╡
#> │ 7        ┆ 65.3    ┆ 84.0    ┆ 0.777 │
...
(표의 `loss_mn`·`prem_mn`은 백만원 단위 누적 손해·누적 보험료, `ratio`는 누적
손해율입니다. 코드의 열 이름은 정렬이 깨지지 않도록 ASCII로 두고, 뜻은 본문에서
풀어 씁니다.)
...
(lf.df
    ...
    .select(
        "duration",
        (pl.col("loss_proj") / 1e6).round(1).alias("loss_mn"),
        (pl.col("incr_loss_proj") / 1e6).round(1).alias("incr_loss_mn"),
    ))
#> shape: (4, 3)
#> ┌──────────┬─────────┬──────────────┐
#> │ duration ┆ loss_mn ┆ incr_loss_mn │
#> │ ---      ┆ ---     ┆ ---          │
#> │ i64      ┆ f64     ┆ f64          │
#> ╞══════════╪═════════╪══════════════╡
#> │ 7        ┆ 65.3    ┆ 12.0         │
...
(pf.df
    ...
    .select(
        "duration",
        (pl.col("premium_proj") / 1e6).round(1).alias("prem_mn"),
    ))
#> shape: (4, 2)
#> ┌──────────┬─────────┐
#> │ duration ┆ prem_mn │
...
#> shape: (5, 4)
#> ┌──────────┬─────────┬─────────┬────────────┐
#> │ duration ┆ loss_mn ┆ prem_mn ┆ ratio_proj │
#> │ ---      ┆ ---     ┆ ---     ┆ ---        │
#> │ i64      ┆ f64     ┆ f64     ┆ f64        │
#> ╞══════════╪═════════╪═════════╪════════════╡
#> │ 7        ┆ 65.3    ┆ 84.0    ┆ 0.777      │
...
#> shape: (4, 5)
#> ┌────────────┬─────────┬─────────┬────────────┬──────────┐
#> │ cohort     ┆ loss_mn ┆ prem_mn ┆ ratio_proj ┆ ratio_se │
#> │ ---        ┆ ---     ┆ ---     ┆ ---        ┆ ---      │
...
#> │ 2025-12-01 ┆ 296.6   ┆ 352.4   ┆ 0.842      ┆ 0.049    │
#> └────────────┴─────────┴─────────┴────────────┴──────────┘
```
