...
(tri.df
    ...
    .select(
        "duration",
        (pl.col("loss") / 1e6).round(1).alias("누적손해_백만"),
        (pl.col("premium") / 1e6).round(1).alias("누적보험료_백만"),
        pl.col("ratio").round(3).alias("누적손해율"),
    ))
#> shape: (1, 4)
#> ┌──────────┬───────────────┬─────────────────┬────────────┐
...
(lf.df
    ...
    .select(
        "duration",
        (pl.col("loss_proj") / 1e6).round(1).alias("누적손해_백만"),
        (pl.col("incr_loss_proj") / 1e6).round(1).alias("증분손해_백만"),
    ))
#> shape: (4, 3)
#> ┌──────────┬───────────────┬───────────────┐
#> │ duration ┆ 누적손해_백만 ┆ 증분손해_백만 │
#> ╞══════════╪═══════════════╪═══════════════╡
#> │ 7        ┆ 65.3          ┆ 12.0          │
#> │ 8        ┆ 75.7          ┆ 10.4          │
#> │ 12       ┆ 114.6         ┆ 8.7           │
...
경과 8의 증분 손해 10.4백만은 강도 `g_8 = 0.109`에 그 경과의 누적 보험료 96.0백만을
곱한 값입니다 (`0.109 x 96.0 ≈ 10.4`). 관측이 끝난 경과 7의 누적 손해 65.3백만에서
출발해, 미래 경과마다 이 증분을 더해 나가면 경과 36에서 누적 손해 365.9백만에
닿습니다.
...
(pf.df
    ...
    .select(
        "duration",
        (pl.col("premium_proj") / 1e6).round(1).alias("누적보험료_백만"),
    ))
#> shape: (4, 2)
#> ┌──────────┬─────────────────┐
#> │ duration ┆ 누적보험료_백만 │
...
(rf.df
    ...
    .select(
        "duration",
        (pl.col("loss_proj") / 1e6).round(1).alias("손해_백만"),
        (pl.col("premium_proj") / 1e6).round(1).alias("보험료_백만"),
        pl.col("ratio_proj").round(3).alias("예측손해율"),
    ))
#> shape: (5, 4)
#> ┌──────────┬───────────┬────────────┬────────────┐
#> │ duration ┆ 손해_백만 ┆ 보험료_백만 ┆ 예측손해율 │
#> ╞══════════╪═══════════╪════════════╪════════════╡
#> │ 7        ┆ 65.3      ┆ 84.0       ┆ 0.777      │
#> │ 8        ┆ 75.7      ┆ 96.0       ┆ 0.789      │
#> │ 12       ┆ 114.6     ┆ 144.1      ┆ 0.796      │
#> │ 24       ┆ 234.9     ┆ 287.9      ┆ 0.816      │
#> │ 36       ┆ 365.9     ┆ 431.6      ┆ 0.848      │
#> └──────────┴───────────┴────────────┴────────────┘
```
...
(rf.summary()
    .filter(pl.col("cohort").is_in([
        pl.date(2023, 1, 1), pl.date(2024, 6, 1),
        pl.date(2025, 6, 1), pl.date(2025, 12, 1)]))
    .select(
        "cohort",
        (pl.col("loss_proj") / 1e6).round(1).alias("손해_백만"),
        (pl.col("premium_proj") / 1e6).round(1).alias("보험료_백만"),
        pl.col("ratio_proj").round(3).alias("예측손해율"),
        pl.col("ratio_se").round(4).alias("손해율_표준오차"),
    ))
#> shape: (4, 5)
#> ┌────────────┬───────────┬────────────┬────────────┬─────────────────┐
#> │ cohort     ┆ 손해_백만 ┆ 보험료_백만 ┆ 예측손해율 ┆ 손해율_표준오차 │
#> ╞════════════╪═══════════╪════════════╪════════════╪═════════════════╡
#> │ 2023-01-01 ┆ 146.8     ┆ 169.5      ┆ 0.866      ┆ null            │
#> │ 2024-06-01 ┆ 248.9     ┆ 282.2      ┆ 0.882      ┆ 0.0365          │
#> │ 2025-06-01 ┆ 365.9     ┆ 431.6      ┆ 0.848      ┆ 0.0414          │
#> │ 2025-12-01 ┆ 296.6     ┆ 352.4      ┆ 0.842      ┆ 0.0490          │
#> └────────────┴───────────┴────────────┴────────────┴─────────────────┘
```
...
(rf.df
    ...
    .select(
        "duration",
        pl.col("ratio_proj").round(3).alias("예측손해율"),
        pl.col("ratio_se").round(4).alias("표준오차"),
        pl.col("ratio_ci_lo").round(3).alias("하한"),
        pl.col("ratio_ci_hi").round(3).alias("상한"),
    ))
#> shape: (4, 5)
#> ┌──────────┬────────────┬──────────┬───────┬───────┐
#> │ duration ┆ 예측손해율 ┆ 표준오차 ┆ 하한  ┆ 상한  │
#> ╞══════════╪════════════╪══════════╪═══════╪═══════╡
