...
데이터를 재표집해 적합을 수없이 반복하는 시뮬레이션)은 그 가정 없이
예측치의 *경험적 분포*를 직접 그린다.

불확실성은 모형에 `uncertainty=`로 끼워 넣는다. 세 가지가 있다.
...
* - 전략
  - 방식
* - `lr.Analytical`
  - 공식 기반(기본값). Mack의 분산 분해로 닫힌형 SE를 구한다
* - `lr.ResidualBootstrap`
  - 잔차를 재표집 -> 가상의 삼각형 재구성 -> 다시 적합을 수백 번 반복
* - `lr.ParametricBootstrap`
  - 추정한 분포의 모수에서 직접 재표집해 반복
```
