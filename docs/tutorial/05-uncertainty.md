...
- **과정오차**(process error) — 미래의 손해 자체가 **확률적**이다. 설령 참된
  진전 인자를 정확히 안다 해도, 발생하는 손해는 그 평균 주위로 흩어진
  무작위성에 의존한다.
...
.. plot::
   ...
   tri = lr.Triangle(df, groups="coverage", grain="Q")
   s = (
       lr.LossRatio(loss=lr.ChainLadder(), premium=lr.PooledPremium())
       .fit(tri)
       .summary()
       .sort("cohort")
...
df = lr.load_experience().filter(pl.col("coverage") == "SURGERY")
tri = lr.Triangle(df, groups="coverage", grain="Q")
fit = lr.LossRatio(loss=lr.ChainLadder(), premium=lr.PooledPremium()).fit(tri)
...
채우고 `n_jobs` 병렬도 그대로다. 잔차를 버리고 다시 뽑지 않으니 관측 셀이 적은 경과·
접목(graft) 꼬리에서 잔차 풀 고갈 없이 매끄럽게 도는 대신, 보통 잔차
...
(관측 셀이 적은 경과는 그 경과 군집 안에서, 잔차가 `min_pool`(기본 5)개 미만이면 전역
풀에서 재표집한다.)
...
**달력축 랜덤워크-드리프트 밴드**(calendar RW-with-drift)까지 얹는다 -- 복제마다 달력
추세율을 하나 뽑아 프런티어 너머 셀에 경과가 멀수록 크게 누적시켜, 달력연도 추세가 점
추정과 다르게 흘러갈 위험을 반영한다. 세그먼트별 복제는 서로 독립이라
...
* - `uncertainty=lr.WeightedBootstrap(...)`
  - 가중(FRW, fractional-random-weight) 부트스트랩. 잔차를 다시 뽑는 대신 셀마다 무작위 가중치를 실어 적합을 수백 번. 같은 SE/CI 컬럼을 채우고, 관측 셀이 적은 꼬리에서 잔차 풀 고갈이 없다(준비금 검증 전 실험적, 보통 살짝 넓다)
```
...
- {doc}`API 레퍼런스 <../api>`의 `LossRatio`, `ChainLadder`, `PooledLoss`,
