...
풀링(`PooledLoss`)은 1.30, link ratio(`LinkRatio`)는 0.88로. 그런데 숫자
...
채워진다(`.to_polars()`로 볼 수 있다) — Mack의 분산 분해를 닫힌형으로
...
.. plot::
   ...
   tri = lr.Triangle(df, groups="coverage", grain="Q")
   s = (
       lr.Ratio(loss=lr.LinkRatio(), premium=lr.PooledPremium())
       .fit(tri)
       .summary()
       .sort("cohort")
...
df = lr.load_experience().filter(pl.col("coverage") == "SURGERY")
tri = lr.Triangle(df, groups="coverage", grain="Q")
fit = lr.Ratio(loss=lr.LinkRatio(), premium=lr.PooledPremium()).fit(tri)
...
def recent_cv(se_method, rho=0.0):
    fit = lr.Ratio(
        loss=lr.LinkRatio(), premium=lr.PooledPremium(),
...
지금까지의 표준오차·신뢰구간은 **기본값인 분석적 밴드**(Mack의 분산 분해)에
...
* - 전략
  - 방식
* - 기본값 (인자 없음)
  - 분석적 밴드. Mack의 분산 분해로 닫힌형 SE + 정규 근사 신뢰구간. 가장 빠르다
...
- {doc}`API 레퍼런스 <../api>`의 `Ratio`, `LinkRatio`, `PooledLoss`,
