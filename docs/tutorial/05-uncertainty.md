...
`se_method="delta"`를 쓴다. `rho`는 (-1, 1) 범위의 손해-보험료 상관이고,
직접 짐작하기 어려우면 **`rho="auto"`**로 데이터에서 그룹별로 추정시킨다(손해·
보험료 상대 잔차의 상관). 위 `0.9`는 효과를 보이려는 예시값이고, 이 합성
데이터의 auto 추정치는 ~0.04로 작다 — 실데이터에선 담보별 중앙값 ~0.2 안팎의
양수로 나온다(손해가 보험료로 지어져 같이 움직이기 때문).
```
