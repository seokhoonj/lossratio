...
#> │ ---        ┆ ---      ┆ ---      ┆ ---      ┆ ---     ┆ ---       │
#> │ date       ┆ i64      ┆ i64      ┆ i64      ┆ i64     ┆ f64       │
#> ╞════════════╪══════════╪══════════╪══════════╪═════════╪═══════════╡
#> │ 2024-01-01 ┆ 8        ┆ 28976401 ┆ 29109870 ┆ -133469 ┆ -0.004585 │
#> └────────────┴──────────┴──────────┴──────────┴─────────┴───────────┘
```

이 칸에서 실제 누적 손해는 $28,976,401, 가린 데이터 없이 예측한 값은
$29,109,870이다. 같은 칸을 두 지표가 이렇게 다르게 요약한다.

- **AEG** = 28,976,401 − 29,109,870 = **−$133,469**. 모델이 약
  $133,000 과대 예측했다는 *절대 금액*이다.
- **A/E Error** = 28,976,401 / 29,109,870 − 1 = **−0.46%**. 같은 격차를
  예측값 대비 *비율*로 떼어 낸 값이다.

약 $29,000,000짜리 칸에서 $133,000 격차는 −0.46%에 지나지 않는다 — 절대 금액으로는
큰 숫자가 상대 오차로는 작다. 그래서 절대 금액 영향이 궁금하면 AEG를, 큰 칸과
...
분모쪽 `PooledPremium`을 묶은 `LossRatio`로 만든다.
...
```python
losses = {"pooled": lr.PooledLoss, "chain ladder": lr.ChainLadder, "credible": lr.CredibleLoss}
for name, loss in losses.items():
    est = lr.LossRatio(loss=loss(), premium=lr.PooledPremium())
    bt = lr.Backtest(estimator=est, holdouts=3, target="ratio").fit(tri)
    err = float(bt.ae_err["ae_err"].abs().mean()) * 100
    print(f"{name:>12}: 평균 |A/E Error| = {err:>4.1f}%")
#>       pooled: 평균 |A/E Error| = 11.7%
#> chain ladder: 평균 |A/E Error| =  4.5%
...
미래*까지 예측하게 하면, 관측 셀이 적은 구간에 외삽하는 `ChainLadder`가 흔들리고 노출에
고정한 `PooledLoss`가 다시 안전해진다. 검증 결과는 늘 hold-out 길이와 함께 읽어야 한다.
```

같은 방식으로 regime 반영 여부(5장)도 백테스트로 비교할 수 있다 —
`lr.Backtest(estimator=lr.LossRatio(loss=lr.PooledLoss(regime=date(2024, 7, 1)),
...
#> │ str      ┆ i64     ┆ u32 ┆ f64       ┆ f64          │
#> ╞══════════╪═════════╪═════╪═══════════╪══════════════╡
#> │ SURGERY  ┆ 1       ┆ 21  ┆ -0.046663 ┆ 0.067953     │
#> │ SURGERY  ┆ 2       ┆ 18  ┆ -0.077085 ┆ 0.109897     │
#> │ SURGERY  ┆ 3       ┆ 8   ┆ -0.081963 ┆ 0.120106     │
#> │ SURGERY  ┆ 4       ┆ 6   ┆ -0.117574 ┆ 0.166892     │
#> └──────────┴─────────┴─────┴───────────┴──────────────┘
```
...
#> ╞══════════╪═════════════════╪═════╪═══════════╪══════════════╡
#> │ SURGERY  ┆ 1               ┆ 11  ┆ -0.169082 ┆ 0.223112     │
#> │ SURGERY  ┆ 2               ┆ 10  ┆ -0.155243 ┆ 0.195684     │
#> │ SURGERY  ┆ 3               ┆ 9   ┆ -0.032847 ┆ 0.047931     │
#> │ SURGERY  ┆ 4               ┆ 8   ┆ -0.035369 ┆ 0.050083     │
#> │ SURGERY  ┆ 5               ┆ 6   ┆ 0.000543  ┆ 0.005407     │
#> │ SURGERY  ┆ 6               ┆ 4   ┆ 0.001107  ┆ 0.012868     │
#> │ SURGERY  ┆ 7               ┆ 3   ┆ 0.006005  ┆ 0.008968     │
#> │ SURGERY  ┆ 8               ┆ 2   ┆ 0.000837  ┆ 0.002513     │
#> │ SURGERY  ┆ 9               ┆ 1   ┆ -0.008475 ┆ 0.012791     │
#> └──────────┴─────────────────┴─────┴───────────┴──────────────┘
```

`anchor_duration`이 작은(최근 코호트) 칸은 편향이 −15~−17%로 크다가, 5경과부터
...
가장자리의 표본이 적은 칸 몇 개(여기서 9경과는 `n`이 1뿐이다)가 우연히 범위 안에 들어와
...
```{admonition} EstimatorComparison과 ProjectionOverlay는 서로 다른 질문
:class: note

두 클래스는 라벨 붙인 추정기 묶음을 받는 같은 문법을 쓰지만 답하는 질문이 다르다.
`EstimatorComparison`은 **표본 밖(out-of-sample)** 질문이다 — hold-out으로 데이터를
가리고 재적합해, 가리지 않은 셀에서 A/E 오차로 "어느 방법이 못 본 데이터를 더
정확히 맞혔나"를 잰다. 3장에서 본 `ProjectionOverlay`는 **표본 안(in-sample)**
질문이다 — 각 추정기를 전체 데이터에 적합해 관측 최대 경과까지의 예측 궤적을 겹쳐
"각 방법이 이 코호트를 어디로 예측하나"를 보여 준다. 정확도 판정에는 이 절의
`EstimatorComparison`을, 예측이 갈라지는 모습을 눈으로 보려면 `ProjectionOverlay`를
쓴다.
```
...
cmp = lr.EstimatorComparison(
    {
        ...
        "chain ladder": lr.LossRatio(loss=lr.ChainLadder(),  premium=lr.PooledPremium()),
...
#> │ chain ladder ┆ 21  ┆ 0.039402  ┆ 0.045237 ┆ 0.106014 ┆ 0.952381    ┆ 0.952381    │
#> │ credible     ┆ 21  ┆ -0.111574 ┆ 0.113687 ┆ 0.173219 ┆ null        ┆ null        │
#> └──────────────┴─────┴───────────┴──────────┴──────────┴─────────────┴─────────────┘
```
...
- **편향**: `bias`(부호 있는 평균 A/E Error). `pooled`/`credible`는 −0.115/
  −0.112로 과소 예측, `chain ladder`는 +0.039로 살짝 과대 예측이다.
- **분산**: `mae`(평균 |A/E Error|)와 `rmse`. `chain ladder`가 0.045로 가장 작다
  (증분 lane에서는 `deviance`도 함께 산출되나 누적 lane에선 `null`이다).
- **구간 정직성**: `coverage_80`/`coverage_95`(예측 구간이 실제값을 덮은 비율).
  명목 80% 구간을 `pooled`는 43%만 덮어 과신하고, `chain ladder`는 95%로 넉넉하다.
...
#> │ SURGERY  ┆ chain ladder ┆ 3       ┆ 21      ┆ 21        ┆ 0         │
...
#> ╞══════════╪═════════╪══════════════╪═════╪══════════╪══════╡
#> │ SURGERY  ┆ 3       ┆ chain ladder ┆ 9   ┆ 0.005676 ┆ 1    │
#> │ SURGERY  ┆ 3       ┆ pooled       ┆ 9   ┆ 0.005717 ┆ 2    │
#> │ SURGERY  ┆ 3       ┆ credible     ┆ 9   ┆ 0.007072 ┆ 3    │
#> └──────────┴─────────┴──────────────┴─────┴──────────┴──────┘
```

전체로 보면 `chain ladder`가 `pooled`를 크게 앞섰지만(mae 0.045 대 0.117), 결정
...
#> │ coverage ┆ holdout ┆ estimator    ┆ n   ┆ mae      ┆ rank │
#> ╞══════════╪═════════╪══════════════╪═════╪══════════╪══════╡
#> │ SURGERY  ┆ 3       ┆ chain ladder ┆ 21  ┆ 0.045237 ┆ 1    │
#> │ SURGERY  ┆ 3       ┆ credible     ┆ 21  ┆ 0.113687 ┆ 2    │
...
#> ╞══════════╪═════════╪══════════════╪═════╪═══════════╪══════╡
#> │ SURGERY  ┆ 3       ┆ chain ladder ┆ 21  ┆ 0.039402  ┆ 1    │
...
#> │ SURGERY  ┆ 3       ┆ chain ladder ┆ 21  ┆ 0.952381    ┆ 1    │
#> │ SURGERY  ┆ 3       ┆ pooled       ┆ 21  ┆ 0.428571    ┆ 2    │
#> └──────────┴─────────┴──────────────┴─────┴─────────────┴──────┘
```

`mae`로도 `bias`로도 `chain ladder`가 1등이지만, `coverage_80`에서는 점추정인
`credible`이 `null`로 빠지고 `chain ladder`가 1등이다(`pooled`의 좁은 구간은 2등).
...
#> ╞══════════╪═════════╪══════════════╪═══════════╪══════════╪══════════╡
#> │ SURGERY  ┆ 3       ┆ chain ladder ┆ 1.0       ┆ 1.0      ┆ 2.0      │
...
`chain ladder`가 승자다. 7.5의 손계산이 "chain ladder가 평균 오차 최소"라는 한 가지
...
#> ╞══════════╪══════════════╪══════════════╪══════════════╪═════════════╪════════════════╪═════════╪═════════════╡
#> │ SURGERY  ┆ chain ladder ┆ null         ┆ chain ladder ┆ null        ┆ chain ladder   ┆ 1       ┆ 5           │
#> └──────────┴──────────────┴──────────────┴──────────────┴─────────────┴────────────────┴─────────┴─────────────┘
```
