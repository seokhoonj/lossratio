...
몰리는 경우가 많아, 초기에는 손해율이 빠르게 오르다 차차 안정됩니다.
...
아니라, **손해가 경과에 따라 어떻게 발전하는지의 양상**을 먼저
파악하는 것입니다. lossratio는 이 발전 양상을 코호트 삼각형으로
정리하는 데서 시작합니다.

## 1.3 경험 데이터의 구조

분석의 원재료는 **경험 데이터(experience data)**입니다. 한 행이
"어떤 코호트가, 경과 몇 개월 시점에, 그 기간 동안 얼마의 손해와
보험료를 냈는가"를 담은 long-format 표입니다.
...
이번 튜토리얼은 구조 변화가 한 번 심어져 있는 SUR 담보에 집중하겠습니다.
...
`ratio`는 경과에 따라 완만하게 자라지만, `incr_ratio`는 그 기간만의
