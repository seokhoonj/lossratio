...
삼각형은 이미 집계되어 공변량 컬럼을 잃었으므로, 적합 시점에 *쪼개지기 전의*
원본 데이터를 따로 읽어 셀을 복원한다.
...
`lam_cov="auto"` 가 중요한 까닭. 레벨이 많고 데이터가 드문 공변량을 수축 없이
적합하면, 데이터가 거의 없는 레벨에서 상대도가 분리(separation -- 그 레벨만
손해/무손해로 갈려 추정이 무한대로 튐)로 폭발한다.
