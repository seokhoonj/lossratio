...
- 손해율의 한 칸 발전 `rho_k = f^L_k / f^P_k`로 정착을 직접 재는 게이트
...
장기건강보험은 손해도 보험료도 끝없이 흐릅니다 — *다 자란 최종값*이 없습니다.
그래서 관측이 끝난 경과 너머의 손해율은 *정교한 꼬리 곡선*으로 외삽하는 게
아니라, **발전이 이미 정착했다면 마지막 손해율을 그대로 평탄 연장**하는 것이
정직합니다(실데이터 표본 외 검증의 결론). 단 **정착했을 때만** 그렇습니다.

정착했는지는 눈으로 가리기 어렵습니다. 1장에서 본 **관성(inertia)** — 누적
손해율이 분모효과에 눌려 굼뜨게 움직이는 성질 — 때문에, 누적값은 *아직 발전
중인데도* 평평해 보일 수 있기 때문입니다. 안정성 판정은 그 관성에 속지 않게,
손해율의 *한 칸 발전*을 직접 봅니다.

## 8.2 어떻게 판정하나 — 손해율의 한 칸 발전

손해율 $R_k = L_k / P_k$가 평평하다는 것은, **분자(손해)와 분모(보험료)가 같은
속도로 발전한다**는 뜻입니다. 그래서 한 칸 발전을 손해 link 인자와 보험료
link 인자의 비로 봅니다.

$$\rho_k = \frac{f^L_k}{f^P_k},\qquad f^L_k = \frac{\sum_i L_{i,k+1}}{\sum_i L_{i,k}},\quad
f^P_k = \frac{\sum_i P_{i,k+1}}{\sum_i P_{i,k}}$$

손해와 보험료가 발맞춰 자라면 $\rho_k \to 1$ — 손해율이 평평합니다. 최근
`window`개 경과의 $|\rho_k - 1|$이 모두 문턱 `tol` 아래면 **안정(stable)**,
아니면 **발전 중(developing)**, 경과가 너무 얕으면 **`insufficient_depth`**
입니다.

```{admonition} 왜 손해율이 손해·보험료보다 안정한가
:class: note

손해율은 손해·보험료 *각각*보다 훨씬 안정합니다. 둘이 함께 발전하므로
**분자와 분모의 변동이 비에서 상쇄**되기 때문입니다(5장의 `se_method="fixed"`가
기댄 바로 그 상쇄). 그래서 관측 너머 go-forward도, 손해·보험료를 따로
외삽해 나누기보다 **손해율을 통째로 동결**하는 편이 실데이터에서 더
정확합니다.
```

옛 누적값을 다시 적합해 가며 보는 대신, 이렇게 **관측된 발전을 직접** 재므로
판정이 가볍고, "관측은 평평해 보여도 발전은 안 멈췄다"를 정직하게 가립니다.
...
누적 손해율 $R_k$의 경과별 궤적을 보면, 안정 담보는 끝에서 평평해지고 발전
중인 담보는 아직 오릅니다.
...
안정 담보는 마지막 손해율을 평탄 동결(`frozen`), 발전 중인 담보는 값을 비우고
...
움직임을 가린 것뿐, 손해율은 아직 발전 중일 수 있습니다. 안정성은 누적값이
아니라 **한 칸 발전 $\rho_k$**를 보므로 이 관성에 속지 않습니다 — "관측은
안정처럼 보여도 발전은 안 멈췄다"를 정직하게 `developing`으로 답합니다.
```
...
*얼마나* 믿나)** 으로 판단합니다 — 발전 중인 담보일수록 CI가 넓게 나와 그
...
- {doc}`4장 — 손해율 예측 <04-projection>`: 안정성이 신뢰하는 그 예측을 세우는
  강도 사다리(Pooled·Credible·Smooth)와 벤치마크 LinkRatio.
