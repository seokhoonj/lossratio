...
빈 칸을 채우는 방법은 셋이다. 셋은 대립하는 별개 방법이 아니라 *관측된
진전 패턴을 얼마나 믿느냐*의 스펙트럼 위에 놓인다.

- **노출 기반(exposure-driven, ED)** — 미래 손해를 *위험보험료*에 고정한다.
  강도를 보험료에 곱한 증분을 더해 나간다. 어느 경과에서 갈아탈지
  따로 판단할 필요가 없어 **무조건 안전한 기본값**이다. 그래서 method
  기본값이 `"ed"`다.
- **체인래더(CL)** — 관측된 손해의 *진전 패턴*만으로 외삽한다. 누적 손해에
  ATA 인자를 곱해 나가는 고전적 대안이다. 충분히 경과한 구간에선 정확하지만,
  얇은 초기 관측에 곱셈을 거듭하면 출렁인다.
- **단계 적응형(stage-adaptive, SA)** — ED와 CL을 경과에 따라 **갈아타는**
  합성이다. 초기 구간은 ED로 안전하게, 진전이 신뢰할 만해진 뒤엔 CL로
...
flowchart TD
  ...
  Q -->|"고전적 대안 · 진전이 신뢰할 만할 때"| CL["CL (체인래더)<br/>누적 손해 x ATA 인자"]
...
* - ED
  ...
  - 안전한 기본값. 전환점 판단 불필요, 초기 ATA 변동에 강건
* - CL
  - 누적 손해 x ATA 인자
  - 고전적 대안. 진전이 일관될 때 정확, 최근 코호트엔 위험
* - SA
  - 전환 전 ED + 전환 후 CL
...
ED는 초기 구간에서 안정적이고, CL은 진전이 일관된 구간에서 손해의 진전을
...
```text
  ...
  SA: ED before the switch k, CL at and after k (here k = 4)

       duration   1   2   3   4   5   6
       stage      ED  ED  ED  CL  CL  CL
                              ^ switch k
...
.. plot::
   :context: close-figs
   :caption: 같은 최근 수술담보 코호트(2025-1분기)에 SA(switch=3)를 적용. 경과 3까지는 ED로 안전하게 채우고, 경과 3부터 CL로 갈아타 예측이 CL 쪽(약 0.88)으로 수렴한다. ED와 CL 사이의 절충이다.
   ...
   ax.plot(duration, cl["r_proj"].to_list(), "--", color="#d62728", alpha=0.4, label="CL")
   ax.plot(duration, sa["r_proj"].to_list(), "-", color="#2ca02c", lw=2, label="SA(switch=3)")
...
경계마다 SA 모형을 만들어 hold-out 손해 예측 오차를 비교하고, 그 오차를 가장
...
데이터가 두꺼워지면 실제 전환이 잡힌다. 같은 수술담보를 **월 단위 전체
...
#> │ coverage  ┆ point ┆ status │
#> ╞═══════════╪═══════╪════════╡
#> │ CANCER    ┆ null  ┆ ed     │   <- 검증된 순수 ED
#> │ CI        ┆ null  ┆ ed     │
#> │ INPATIENT ┆ null  ┆ ed     │
#> │ SURGERY   ┆ 1     ┆ cl     │   <- 데이터가 두꺼워지자 CL이 잡힘
#> └───────────┴───────┴────────┘
```
...
SA는 이 스펙트럼 위에서 **갈아타는 길**이다. 진전을 아직 못 믿을 초기엔
ED로 안전하게, 진전이 코호트 간 일관되게 안정된 뒤엔 CL로 정확하게 — 그
...
* - CL
  - 충분히 경과해 진전 패턴이 코호트 간 일관될 때. 최근 코호트엔 위험
* - SA
  - 초기 ED 안정 + 후기 CL 진전을 한 번에. 전환 근거가 데이터로 분명할 때 (SwitchPoint)
```

기본값 `"ed"`로 시작해, 진전이 신뢰할 만하다는 근거가 백테스트로 확인되면
