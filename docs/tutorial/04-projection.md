# 4장. 손해율 예측 — PooledLoss · CredibleLoss
...
이 장의 주인공은 둘이다 — 베이스라인 **PooledLoss**와, 그 위에 코호트별
수준 보정을 얹는 **CredibleLoss**. 거기에 고전적 참조점으로 **LinkRatio**가
함께 등장한다.

- **PooledLoss(완전 풀링, 노출 기반)** — 미래 손해를 *위험보험료*에
...
- **LinkRatio(체인래더)** — 관측된 손해의 *진전 패턴*만으로 외삽한다.
  누적 손해에 ATA 인자를 곱해 나가는 Mack(1993) 계열의 **고전적
...
flowchart TD
  ...
  Q -->|"기본값 · 무조건 안전"| ED["PooledLoss (노출 기반)<br/>강도 x 보험료"]
  Q -->|"코호트 수준 보정"| CS["CredibleLoss (부분 풀링)<br/>코호트 스케일 x 강도 x 보험료"]
  Q -->|"고전적 참조점"| CL["LinkRatio (체인래더)<br/>누적 손해 x ATA 인자"]
  CS -.->|"형상은 PooledLoss 그대로<br/>수준만 보정"| ED
...
같은 빈 칸을 LinkRatio로 채워 본다. ATA 인자는 2장에서 `f₁ = 1.5`, `f₂ = 1.2`였다.
LinkRatio는 **직전 누적 손해에 인자를 곱하면** 채워진다.
...
```python
lr.LinkRatio().fit(tri).df.select(["cohort", "duration", "loss_proj"]).sort(
    ["cohort", "duration"])
#> ┌────────────┬──────────┬───────────┐
#> │ cohort     ┆ duration ┆ loss_proj │
#> ╞════════════╪══════════╪═══════════╡
...
LinkRatio와 PooledLoss를 나란히 두면 흥미로운 일이 보인다.
...
경과 2에서 B의 누적 손해(180)가 A(150)보다 크다. LinkRatio는 그 높은 수준을
...
- **LinkRatio**의 증분은 코호트 *자신이 쌓은 누적 손해*에 비례한다 — 초기
...
LinkRatio는 손해보험 reserving의 표준 어휘라, 업계 관행과 결과를 **소통하고
검증하는 참조점**으로서의 가치가 크다. 다만 장기 건강보험 실데이터의
표본 외(out-of-sample) 비교에서는 노출 기반 베이스라인이 내다보는 거리
전반에서 더 잘 맞았다 — LinkRatio가 *내 데이터*에서 실제로 보탬이 되는지는
...
.. plot::
   ...
   _MODELS = {"pooled": lr.PooledLoss, "linkratio": lr.LinkRatio}
...
.. plot::
   :context: close-figs
   :caption: 최근 수술담보 코호트(2025-1분기, 경과 1-4만 관측)의 누적 손해율 예측. 점선 오른쪽이 예측 구간이다. PooledLoss는 보험료에 고정해 포트폴리오 수준(약 1.30)으로 올라가고, LinkRatio는 얇은 관측만으로 외삽해 약 0.88에 머문다.
...
약 1.30, LinkRatio는 약 0.88 — 한 코호트의 예측 손해율 추정이 방법에 따라
크게 차이 난다. 얇은 관측에 곱셈을 거듭하는 LinkRatio는 **반응성**이 높은
...
PooledLoss와 LinkRatio는 대립하는 두 방법이 아니라 **하나의 축 위
...
- **PooledLoss** — 진전 대신 **노출(보험료)에 고정**한다. 게다가 강도
  ...
  노출 패턴 유지를 가정).
- **LinkRatio** — 진전을 전적으로 믿는다. 코호트가 쌓은 누적 손해에 ATA
...
LinkRatio)과 수준의 축(풀링 vs 코호트별)이 별개라는 것이, 이 장 방법들의
가장 간결한 지도다.
...
- {doc}`API 레퍼런스 <../api>`의 `PooledLoss`, `CredibleLoss`, `LinkRatio`, `Ratio`
