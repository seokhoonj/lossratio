...
- **노출 기반(exposure-driven, ED)** — 미래 손해를 *보험료라는 바깥 기준*에
  고정한다. 강도를 보험료에 곱한 증분을 더해 나간다. 어느 경과에서 갈아탈지
...
.. plot::
   :context: close-figs
   :caption: 어린 수술담보 코호트(2025-1분기, 경과 1-4만 관측)의 누적 손해율 예측. 점선 오른쪽이 예측 구간이다. ED는 보험료에 고정해 포트폴리오 수준(약 1.30)으로 올라가고, CL은 얇은 관측만으로 외삽해 약 0.88에 머문다.
...
```text
  ...
  SA: ED before the switch duration k, CL at/after
       duration:  1    2    3   [k]    4    5    6
                  |---- ED ----|  |---- CL ----|
...
.. plot::
   :context: close-figs
   :caption: 같은 어린 수술담보 코호트(2025-1분기)에 SA(switch=3)를 적용. 경과 3까지는 ED로 안전하게 채우고, 경과 3부터 CL로 갈아타 예측이 CL 쪽(약 0.88)으로 수렴한다. ED와 CL 사이의 절충이다.
