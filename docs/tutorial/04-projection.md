...
다듬는다. PooledLoss의 강도 $g_k$는 경과별로 따로(saturated) 추정하므로, 관측이
두꺼운 큰 book에선 그 형상이 이미 매끄럽다 — 그런 곳에선 SmoothLoss와
PooledLoss가 사실상 겹친다(앞 절들이 SURGERY 같은 큰 book을 쓴 이유다).
...
암(CANCER)담보의 한 채널(TM) 세그먼트로 보면 분명하다. 코호트가 적은 후기
경과에서 풀링 강도(파란 점)가 튀는데, SmoothLoss(초록 선)는 그 추세만 남기고
잡음을 걷어낸다.

```{eval-rst}
.. plot::
   :context: close-figs
   :caption: 얇은 세그먼트(암담보 · TM 채널)의 경과별 강도 g_k(로그 축). 코호트가 적은 후기 경과에서 풀링 강도(파란 점)가 들쭉날쭉하지만, SmoothLoss(초록 선)는 P-spline으로 추세만 남긴다 — 후기 경과(12개월~)의 곡률 잡음이 약 38배 줄어든다.

   import polars as pl
   import lossratio as lr
   import matplotlib.pyplot as plt

   seg = lr.load_experience().filter(
       (pl.col("coverage") == "CANCER") & (pl.col("channel") == "TM"))
   tri_seg = lr.Triangle(seg, groups=["coverage", "channel"])
   newest = tri_seg.df["cohort"].max()

   def _gcurve(est):
       d = (est.fit(tri_seg).df
            .filter(pl.col("cohort") == newest).sort("duration")
            .with_columns(pl.col("premium_proj").shift(1).alias("_L")))
       d = d.with_columns(
           (pl.col("incr_loss_proj") / pl.col("_L")).alias("_g")).drop_nulls("_g")
       return d["duration"].to_list(), d["_g"].to_list()

   dp, gp = _gcurve(lr.PooledLoss())
   ds, gs = _gcurve(lr.SmoothLoss())

   fig, ax = plt.subplots(figsize=(6.2, 3.6))
   ax.plot(dp, gp, "o", color="#1f77b4", ms=4, alpha=0.75,
           label="PooledLoss: pooled $g_k$ (raw)")
   ax.plot(ds, gs, "-", color="#2ca02c", lw=2,
           label="SmoothLoss: smoothed $g_k$")
   ax.set_yscale("log")
   ax.set_xlabel("duration (months)")
   ax.set_ylabel(r"intensity $g_k$ (log scale)")
   ax.legend()
```

```{admonition} 정직하게 — 이 합성 book엔 보탬이 적다
:class: note

위 대비는 *얇은 세그먼트의 후기 경과*에서만 두드러진다. 큰 매끄러운
book(앞 절의 SURGERY 등)에서는 풀링 강도가 이미 매끄러워 SmoothLoss와
PooledLoss의 예측 손해율이 셀 단위로 거의 같다(중앙값 0%). 그러니 SmoothLoss를
일괄 채택하지 말고, *형상이 거친 book*에서만 — 7장 백테스트로 보탬을 확인한 뒤
