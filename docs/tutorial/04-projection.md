...
두꺼운 큰 담보에선 그 형상이 이미 매끄럽다 — 그런 곳에선 SmoothLoss와
PooledLoss가 사실상 겹친다(앞 절들이 SURGERY 같은 큰 담보을 쓴 이유다).
...
출처(`source`: 관측 `observed` / 자기 외삽 `own` / 차용 `borrowed`)를 붙이고,
표준오차·신뢰구간 열은 덜어 낸 가벼운 뷰다.
