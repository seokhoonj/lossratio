...
- **SmoothLoss** — 풀링된 *형상(shape)*을 매끄럽게 다듬는다. 경과마다 자유롭게
  추정한 강도 `g_k`가 관측 셀이 적은 세그먼트에서 들쭉날쭉할 때, P-spline으로 그 형상에
  추세만 남긴다.
...
두 보정 모두 **관측 기간이 짧은 최근 코호트**에서 작동이 또렷하다. 이 장은 내장
...
않고, 코호트 간 분산 `psi`(기본 `"auto"` — Bühlmann-Straub 적률로 추정)에서
...
때, 특히 비중이 크고 형상이 매끄러우면서, 관측 기간이 짧은 최근 코호트가 많은 담보에서 효과가
...
- `Z` — 그 코호트의 신뢰도 **가중치**(위 배합의 $Z$). 노출이 쌓인 초기 코호트는
  `Z`가 1에 가깝고(자기 수준을 거의 그대로 믿는다), 갓 인수되어 관측 셀이 적은 코호트는
...
비중이 큰 담보에선 그 형상이 이미 매끄럽다 — 그런 곳에선 SmoothLoss와
PooledLoss가 사실상 겹친다(앞 절들이 SURGERY 같은 담보를 쓴 이유다).

값이 갈리는 곳은 **관측 셀이 적은 세그먼트의 후기 경과**다. 거기선 한 경과에 모인
...
```{list-table} 관측 셀이 적은 세그먼트(암담보·TM)의 후기 경과별 강도 $g_k$ — 풀링(raw) vs 평활
:header-rows: 1
:widths: 18 28 28
...
*2차 차분의 RMS(제곱평균제곱근, Root Mean Square)*로 정의하면(작을수록 매끄럽다),
위 표의 두 열에서 곧장 나온다.
...
```{admonition} 참고 — 왜 2차 차분이 곡률인가
:class: dropdown

경과별 강도 $g_k$($k = 1, 2, 3, \dots$)를 하나의 곡선으로 보면, **얼마나
휘어 있는가**를 세 단계로 잰다.

- **1차 차분** $\Delta g_k = g_k - g_{k-1}$ 은 **기울기** — 경과 한 칸당 증가량.
- **2차 차분** $\Delta^2 g_k = g_{k+1} - 2g_k + g_{k-1}$ 은 **기울기가 바뀌는
  정도** = 곡률이다. 이산 데이터에서의 2계 도함수 근사로, 직선이면 기울기가
  일정하므로 어디서나 0이 된다.
- **RMS** 는 경과 전체의 2차 차분을 제곱평균한 뒤 제곱근을 씌운다:

  $$\text{curvature} = \sqrt{\frac{1}{N}\sum_k \left(g_{k+1} - 2g_k + g_{k-1}\right)^2}$$

  제곱으로 볼록·오목의 부호를 지우고, 평균으로 경과 개수와 무관하게 만들며,
  제곱근으로 다시 $g_k$와 같은 스케일로 되돌린 **단일 스칼라**다.

예: $g = [0.10,\ 0.14,\ 0.16,\ 0.17,\ 0.17]$ 이면 1차 차분이
$0.04,\ 0.02,\ 0.01,\ 0.00$, 2차 차분이 $-0.02,\ -0.01,\ -0.01$,
제곱합 $0.0006$을 3으로 나눈 $0.0002$의 제곱근 $\approx 0.0141$.

이 곡률은 SmoothLoss가 쓰는 **P-spline 거칠기 벌점**($\sum(\Delta^2)^2$)과
같은 양이다 — 평활은 이 값을 눌러 곡선을 매끄럽게 만들고, RMS 형태는 그 벌점을
경과 개수로 정규화한 진단 지표다.
```
...
위 대비는 *관측 셀이 적은 세그먼트의 후기 경과*에서만 두드러진다. 큰 매끄러운
...
- **`covariates=[...]`** — 코호트 *수준*을 코호트마다 따로 추정하는 대신,
  ...
  관측 셀이 적은 세그먼트의 수준이 그 코호트 하나가 아니라 같은 키를 가진 코호트들에서
...
- **ChainLadder** — 진전을 전적으로 믿는다. 코호트가 쌓은 누적 손해에 ATA
  ...
  코호트엔 정확하지만, 관측 셀이 적은 초기 구간에 곱셈을 거듭하면 출렁인다.
...
* - SmoothLoss
  - 경과별 강도 형상이 들쭉날쭉할 때(관측 셀이 적은 late-tail 세그먼트). 비중이 큰 담보에선 PooledLoss와 사실상 같아 추가 이득이 없음
* - ChainLadder
