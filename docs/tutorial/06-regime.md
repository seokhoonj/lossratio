...
찾은 변화를 적합에 넘기면, 패키지가 그 시점 이전의 코호트를 잘라 내
**새 regime의 코호트만으로 인자를 추정**해 변화 전후를 섞지 않는다. 손해쪽
추정기(`PooledLoss` 등)의 `regime` 인자에 `detect_regime()`이 돌려준
`Regime` 객체를 그대로 넘기면 된다 — 적합 시점에 코호트 컷으로 해소된다.
...
`Regime` 객체**와 **`"auto"`**(적합 직전 자동 탐지)까지 받는다 — 모두 적합
시점에 코호트 컷으로 해소된다. 백테스트에서는 `"auto"`(또는 `Regime.detect(...)`
스펙)가 각 fold의 *마스킹된* 학습 데이터로 다시 탐지되어, 변화 시점이 hold-out
셀을 엿보지 않는다(leakage-safe).
```
