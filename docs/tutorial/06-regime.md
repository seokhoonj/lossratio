...
`detect_regime()`이 돌려준 `Regime` 객체를 그대로 넘기면 된다 — 적합 시점에
...
trimmed = lr.Ratio(                       # detect_regime() 의 Regime 을 그대로 반영
...
#> ╞════════════╪════════════╪════════════════════╡
#> │ 2025-07-01 ┆ 1.374638   ┆ 0.890296           │
#> │ 2025-10-01 ┆ 1.406453   ┆ 0.919844           │
#> └────────────┴────────────┴────────────────────┘
```

값이 갈리는 까닭은 옛 regime 코호트의 손해율이 실제로 높기 때문이다. §6.3에서
...
#> │ cohort     ┆ level    ┆ regime │
#> ╞════════════╪══════════╪════════╡
#> │ 2023-01-01 ┆ 1.187335 ┆ 1      │   ← regime 1 (잘림): 손해율 높음
#> │   …        ┆   …      ┆ 1      │
#> │ 2024-04-01 ┆ 1.032105 ┆ 1      │
#> │ 2024-07-01 ┆ 0.681318 ┆ 2      │   ← regime 2 (유지): 손해율 낮음
#> │   …        ┆   …      ┆ 2      │
#> │ 2025-07-01 ┆ 0.637828 ┆ 2      │
#> └────────────┴──────────┴────────┘
```
...
## 6.5 옛 regime을 버리지 않는 두 방식 — `treatment`

§6.4의 `latest_only`는 가장 단순한 선택 — 옛 regime을 통째로 버린다. 하지만
옛 코호트는 **깊은 경과**까지 관측된 유일한 자료이기도 하다. 새 regime 코호트는
아직 어려서 깊은 경과의 진전을 모르는데, 그 정보를 옛 regime에서 *빌려* 오면
버리지 않고도 변화를 반영할 수 있다. `detect_regime(..., treatment=)`
...
- **`"segment_wise"`** — *모든* regime을 유지한다. 각 regime은 자기 코호트로
  ...
  4장에서 본 `borrowed` 셀 유형이 바로 이 방식에서 나온다.
- **`"covariate"`** — 역시 모든 regime을 유지하되, regime을 *공변량*으로 넣는다.
  하나의 **공유 경과 모양**($g_k$)에 **regime별 수준**을 얹어, 얇은 regime의
...
#> ╞════════════╪═════════════╪══════════════╪═══════════╡
#> │ 2025-04-01 ┆ 0.898271    ┆ 0.918691     ┆ 0.919511  │
#> │ 2025-07-01 ┆ 0.890296    ┆ 0.910535     ┆ 0.929589  │
#> │ 2025-10-01 ┆ 0.919844    ┆ 0.940755     ┆ 0.928666  │
#> └────────────┴─────────────┴──────────────┴───────────┘
```
...
- {doc}`4장 — 손해율 예측 <04-projection>`: regime이 가장 크게 작용하는
  최근 코호트의 예측이 방법마다 갈리는 양상, 그리고 `segment_wise`의
  `borrowed` 셀 유형.
