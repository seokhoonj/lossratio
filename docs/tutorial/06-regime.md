...
```text
 drift (매끄럽게 흐름 -> 직선으로 설명)      step (한 시점에 계단 -> 직선 불가)
손해율                                       손해율
0.70|              o                         0.70|        o  o  o  o
    |           o                                |
0.60|        o                                0.60|
...
base    = lr.Ratio(loss=lr.PooledLoss(), premium=lr.PooledPremium()).fit(tri)                       # regime 무시
trimmed = lr.Ratio(loss=lr.PooledLoss(regime=date(2024, 7, 1)), premium=lr.PooledPremium()).fit(tri) # regime 반영
```
