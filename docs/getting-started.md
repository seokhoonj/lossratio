...
`PooledPremium`을 쓴 뒤, `LossRatio`가 둘을 묶어
...
```python
cl = lr.LossRatio(loss=lr.ChainLadder(), premium=lr.PooledPremium()).fit(tri)
cl.summary().head(2)
#> shape: (2, 6)
#> ┌──────────┬────────────┬───────────┬──────────────┬────────────┬──────────┐
#> │ coverage ┆ cohort     ┆ loss_proj ┆ premium_proj ┆ ratio_proj ┆ ratio_se │
...
```python
bt = lr.Backtest(
    estimator=lr.LossRatio(loss=lr.PooledLoss(), premium=lr.PooledPremium()),
    holdouts=(6, 12),
    target="ratio",
).fit(tri)
bt.reliable_horizon()
...
ec = lr.EstimatorComparison(
    estimators={
        ...
        "chain":    lr.LossRatio(loss=lr.ChainLadder(),  premium=lr.PooledPremium()),
    },
    holdouts=(6, 12),
    target="ratio",
    baseline="pooled",
