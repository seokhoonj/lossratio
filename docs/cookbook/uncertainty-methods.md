...
```{note}
그래서 `Analytical(simulate=True)`는 **CL 전용**입니다. ED 점추정과 짝지어도
...
- **예측분포(히스토그램)가 필요** -> `ResidualBootstrap` 또는
  `ParametricBootstrap`. `ultimate_ratio_samples()`로
  replicate별 포트폴리오 궁극 손해율 벡터를 받아 분위수(p50/p75/p95)를 그립니다.
- **cohort-scaled(CS, `method="cs"`)** -> 자체 잔차 부트스트랩
  (`n_bootstrap=`). 신뢰도로 추정한 코호트 스케일의 분산이 얽혀 닫힌형이
  성립하지 않아 부트스트랩 전용입니다. 과정오차·모수오차 분해는 null로
  남고 총 SE와 분위수 기반 비대칭 CI만 채워지며, `uncertainty=` 오버레이는
  CS에 적용되지 않습니다(거부).
