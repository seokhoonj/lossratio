...
분해 표면(`predict(by=...)`)은 각 코호트 x 경과에서 **전체 예측 누적
...
핵심은 분해 표면이 별도 예측이 *아니라* 전체 예측의 **분배**라는 점이다.
전체 예측은 공변량과 무관하게 정해지고(주변화로 같은 값), 표면은 그
