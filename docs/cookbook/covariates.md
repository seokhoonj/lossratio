...
# 공변량은 어떻게 헤드라인과 맞나 — 유효 강도 붕괴와 분해 표면

9장은 공변량(covariate)을 *쓰는* 법을 다뤘다. 이 레시피는 한 단계 아래 —
공변량을 넣어도 **헤드라인 예측**(headline — 공변량으로 쪼개기 전의 대표
코호트 x 경과 예측, 곧 `predict()` 한 장)이 그대로이고, 분해 표면을 합치면
헤드라인과 *정확히* 일치하는 까닭을 본다. 두 가지 장치 덕분이다: (1) 적합에서 3차원을
2차원으로 **붕괴**시키는 유효 강도, (2) 예측에서 헤드라인을 셀별로
**배분**하는 분해 표면.
...
믹스로 **주변화(marginalize)** 해 코호트별 경과별 **유효 강도** 한 장으로
붕괴시킨다.
...
분해 표면(`predict(by=...)`)은 각 코호트 x 경과에서 **헤드라인 누적
...
비중이 매 경과에서 1로 합산되므로, 표면을 공변량에 대해 합치면
`loss_proj`(와 `premium_proj`)가 **셀 단위로 정확히 재현**된다 — 믹스가
경과에 따라 변해도, 손해만 있고 보험료 없는 셀이 있어도, 좌측 절단
코호트가 있어도. 셀별 손해율은 `헤드라인 손해율 x g_d(x)/g_eff` 라, 상대도가
큰 레벨이 더 높은 손해율로 갈린다.

## 3. 그래서 — 분해는 모델이 아니라 *뷰*

핵심은 분해 표면이 별도 예측이 *아니라* 헤드라인의 **분배**라는 점이다.
헤드라인은 공변량과 무관하게 정해지고(주변화로 같은 값), 표면은 그
헤드라인을 셀별로 나눠 보여줄 뿐이다. 그래서:

- `predict(by=...)` 를 합치면 항상 `predict()` 와 일치한다.
- 공변량을 넣고 빼도 헤드라인 OOS 정확도는 사실상 그대로다.
