...
``` r
summary_ata(x, alpha = 1, digits = 3, ...)
...
- object:
  ...
  An object of class `"ata"`. Used by the S3
  [`summary()`](https://rdrr.io/r/base/summary.html) method, which is
  equivalent to `summary_ata(object, ...)`.

## Value

A `data.table` with class `"ata_summary"` containing one row per ata
link:
