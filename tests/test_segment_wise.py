...
def test_treatment_default_and_validation():
...
def test_segment_wise_keeps_all_regimes_and_borrows_the_tail(Est):
    tri = _tri()
...
@pytest.mark.parametrize("Est", [lr.PooledLoss, lr.CredibleLoss, lr.SmoothLoss])
def test_oldest_regime_equals_standalone_fit_on_its_own_cohorts(Est):
...
def test_segment_wise_cohorts_are_globally_ascending():
    tri = _tri()
    reg_sw = lr.Regime.at(change=CHANGE, treatment="segment_wise")
...
def test_thin_newest_regime_borrows_from_the_start():
...
def test_multi_regime_cascade_three_regimes():
    tri = _tri()
    reg = lr.Regime.at(
