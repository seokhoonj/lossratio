...
removal, a pooled-intensity g->0 tail, etc.) — those are validated separately, not here.
...
def golden_outputs() -> dict[str, pl.DataFrame]:
    ...
    # --- point projections ---
    out["link_ratio"] = _frame(lr.ChainLadder().fit(tri))
    out["pooled"] = _frame(lr.PooledLoss().fit(tri))
    ...
    out["mc_link_ratio"] = _frame(lr.ChainLadder().fit(mc))
    out["mc_pooled"] = _frame(lr.PooledLoss().fit(mc))
...
# analytical SE decomposition. Intentionally outside this intersection: the
