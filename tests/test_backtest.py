...
def test_backtest_invalid_holdout():
    with pytest.raises(ValueError, match="holdout"):
        lr.Backtest(estimator=lr.ChainLadder(), holdout=0, target="loss")
...
def test_backtest_repr():
    bt = lr.Backtest(estimator=lr.ChainLadder(), holdout=2, target="loss").fit(
...
def test_backtest_with_ed_estimator():
    ...
    bt = lr.Backtest(estimator=lr.ExposureDriven(), holdout=1, target="loss").fit(
        lr.Triangle(_toy_triangle_input())
    )
    assert bt.ae_err.shape[0] == 3
...
def test_backtest_pandas_input_mirror():
    ...
    bt = lr.Backtest(estimator=lr.ChainLadder(), holdout=1, target="loss").fit(
        lr.Triangle(df)
    )
