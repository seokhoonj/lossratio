"""_ModelFrame core -- oracle-parity on the exposure (premium) convention.

Reproduces the frozen micro-oracle (tests/test_oracle.py) as a real Triangle
and asserts _ModelFrame feeds the engine the EXACT oracle arrays: per cell
...
def test_columns_and_segment_id():
    mf = _ModelFrame._from_triangle(_oracle_triangle())
...
def test_regime_global_date():
    mf = _ModelFrame._from_triangle(_oracle_triangle(), regime=date(2020, 2, 1))
...
def test_regime_none_and_invalid():
    assert len(_ModelFrame._from_triangle(_oracle_triangle(), regime=None)) == 9
    with pytest.raises(TypeError, match="regime"):
        _ModelFrame._from_triangle(_oracle_triangle(), regime="2020-02-01")
    with pytest.raises(ValueError, match="change must be a date"):
        _ModelFrame._from_triangle(_oracle_triangle(), regime={"A": "2020-02-01"})
...
def test_pooled_g_k_matches_oracle():
    ...
    saturated intensity, computed straight off _ModelFrame's arrays)."""
    mf = _ModelFrame._from_triangle(_oracle_triangle())
...
def test_ungrouped_triangle_segment_id_zero():
    ...
    mf = _ModelFrame._from_triangle(tri)
    assert mf.segments == []
