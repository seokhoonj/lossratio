"""Projected-remaining bar-chart visualisation -- matplotlib backend.

Implements the ``kind="remaining"`` branch of :meth:`LossFit.plot`. The
``kind="projection"`` branch forwards to the shared projection-curve
helper in ``_ratio_vis``.

A horizontal bar chart of per-cohort projected-remaining development
(``loss_proj - latest``) with optional normal-approximation error bars
derived from ``loss_total_se``. Driven entirely by the fit's
``summary()`` table, so it works for any loss fit carrying
``loss_proj_remaining`` / ``loss_total_se``.
"""
...
def plot_cl_remaining(
    ...
    """Per-cohort projected-remaining bar chart, faceted by group."""
    ...
    if "loss_proj_remaining" not in summary.columns:
        raise ValueError(
            "`kind='remaining'` requires a `loss_proj_remaining` column "
            "on `summary()`. Got columns: "
            f"{summary.columns!r}."
        )
    ...
    if isinstance(amount_divisor, str):
        ...
        amount_divisor = _auto_divisor(
            summary["loss_proj_remaining"].to_numpy()
        )
    ...
    for idx, (group_value, sub) in enumerate(facets):
        ...
        remaining = (
            sub["loss_proj_remaining"].to_numpy()[order] / amount_divisor
        )
        labs_ordered = [labels[i] for i in order]

        ax.barh(
            y_positions, remaining,
        ...
        if show_bar:
            ...
            valid = np.isfinite(se) & np.isfinite(remaining)
            if valid.any():
                lower = np.maximum(0.0, remaining - z_alpha * se) - remaining
                upper = remaining + z_alpha * se - remaining
                # `xerr` for barh wants (negative-distance, positive-distance)
                xerr = np.vstack(
                    [-lower[valid], upper[valid]]
                )
                ax.errorbar(
                    remaining[valid], y_positions[valid],
    ...
    title = "Mack Chain Ladder Projected Remaining"
    fig.suptitle(title, fontsize=12, fontweight="bold")
    fig.supxlabel(
        "projected remaining" + (f" ({unit})" if unit else ""), fontsize=10
    )
