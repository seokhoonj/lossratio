...
hlines, a maturity (``mat_k``) reference vline, and the detected
convergence point (``conv_k``) vline. Mirrors the R sibling's
...
def plot_convergence(
    ...
    for ax, (name, values, threshold, y_label) in zip(axes, panels):
        ...
        # mat_k dotted vline
        if conv.mat_k is not None:
            ax.axvline(conv.mat_k, color="grey", linestyle=":", linewidth=0.7)
        # conv_k solid green vline (if found)
        if conv.conv_k is not None and not (
            isinstance(conv.conv_k, float) and np.isnan(conv.conv_k)
        ):
            ax.axvline(conv.conv_k, color="#2ca02c", linestyle="-", linewidth=0.8)
    ...
    subtitle = (
        f"method = {conv.method}   mat_k = {conv.mat_k}   "
        f"conv_k = {conv_k_str}   (max_drift = {conv.max_drift}, "
