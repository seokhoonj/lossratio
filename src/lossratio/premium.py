    ...
    for i in range(len(cohorts)):
        ...
        for k in range(result.n_devs):
            ...
            if row["premium_proj"] is None:
                row["premium_incr_proj"] = None
            elif prev_proj is None:
                row["premium_incr_proj"] = row["premium_proj"]
            else:
                row["premium_incr_proj"] = row["premium_proj"] - prev_proj
            ...
            if (
                ...
                row["premium_ci_lower"] = max(0.0, lo)
                row["premium_ci_upper"] = (
                    row["premium_proj"] + z_alpha * row["premium_total_se"]
                )
            else:
                row["premium_ci_lower"] = None
                row["premium_ci_upper"] = None
    ...
    def summary(self) -> pl.DataFrame:
        ...
        ultimate = (
            ...
            .agg(
                ...
                pl.col("premium_total_se").last().alias("se_ultimate"),
                pl.col("premium_total_cv").last().alias("cv_ultimate"),
            )
            .sort(keys)
        )
