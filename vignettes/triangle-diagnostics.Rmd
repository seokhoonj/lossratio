...
Returns a `triangle_summary` object with mean / median / weighted
...
`summary_ata()` computes per-link statistics that drive maturity
detection:
...
`find_ata_maturity()` operates on a `summary_ata()` object — first
build the descriptive / WLS summary, then probe it for the first
mature link:
...
internal `summary_ata()` step is taken from those callers).
...
`summary_ed()` is the ED-side analogue of `summary_ata()`, computing
per-link statistics for the intensity $g_k = \Delta C^L_k / C^P_k$.
...
Returns a `triangle_validation` object with one row per cohort that
